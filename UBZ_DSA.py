...
# The following code is written by Zhizuo Chen (aka George Chen), for the purpose of self-entertainment only.
...
#A threshold to prevent division by zero.
MIN_DENOMINATOR = 1e-5

#Print the basic settings.
print(f"UBZ_DSA: Golden Ratio = {GOLDEN_RATIO: .8f}, minimum denominator = {MIN_DENOMINATOR} (to prevent division by 0).")
...
#The Conjugate Gradient Descent Algorithm used for calculating A⁻¹b, where A is a symmetric positive definite matrix.
#Reference: https://gregorygundersen.com/blog/2022/03/20/conjugate-gradient-descent/
def cgm(A, b, e = 1e-3):

    #Get the vector dimension.
    dim = A.shape[0]
    #The initial guess of x is zero.
    x = np.random.random(dim)
    #The initial residual (where x0 is 0), and the initial direction of negative gradient (which equals the initial residual).
    r = b - np.dot(A, x)
    d = r + 0.
    #The best guess of {x} that achieves lowest residual (i.e., r = Ax - b) in absolute value.
    bg_x, bg_r_absmax = x + 0., np.abs(r).max()
    
    #Repeat the main loop for at most {A.dimension} times.
    for i in range(dim):

        #The temporary result of Ad & dᵀAd.
        Ad = np.dot(A, d)
        dAd = np.dot(d, Ad)
        #Return if dᵀAd is very small.
        if dAd < MIN_DENOMINATOR: return bg_x
        #Otherwise calculate the optimal size.
        else: ss = np.dot(r, d) / dAd
        #Update the guess of x & the residual.
        x += ss * d
        new_r = r - ss * Ad
        #Calculate the maximum absolute value in the new residual.
        nr_absmax = np.abs(new_r).max()
        #Quits the main loop if the residual is small enough.
        if nr_absmax < e: return x
        #Or update the best guess of {x} if necessary.
        elif nr_absmax < bg_r_absmax: bg_x, bg_r_absmax = x + 0., nr_absmax
        #Compute the next search direction.
        d = new_r + (np.dot(new_r, new_r) / np.dot(r, r)) * d
        #The next residual becomes the current residual in the next step.
        r = new_r
        #for-end
    
    #Return the best guess of {x}.
    return bg_x
    #func-end
